...
- **穷举自查（正文来源 × 校验子来源）**：
  ...
  | 结局 | 磁盘上的正文 | `downloaded_size` | `validator` | 是否自洽 |
  | --- | --- | --- | --- | --- |
  | `open_mode is None`（失败） | `.tmp` 被删除，不存在 | 归零 | 不碰（保留旧值） | 是——旧校验子成了不可达的死值：下一次 `offset` 因 `.tmp` 不存在而重新算成 0，`can_attempt_range` 要求 `offset > 0`，旧校验子永远不会被用来发起续传请求 |
  | `"ab"`（206 续传成功） | 旧前缀 + 本次追加的字节，全部同属校验子标识的那个版本（服务端已用 If-Range 确认版本未变） | 旧偏移起累加 | 不碰（保留旧值） | 是——校验子从未失效过，追加的字节经服务端确认与它同源 |
  | `"wb"`（200 完整正文，有 ETag/Last-Modified） | 全新正文（`wb` 已截断旧内容） | 从 0 起累加 | 覆盖成这次响应头给出的新值 | 是 |
  | `"wb"`（200 完整正文，没有 ETag 也没有 Last-Modified） | 全新正文 | 从 0 起累加 | 覆盖成 `None`（这就是本轮修的坑：此前被误判成“不用管”而跳过覆盖） | 是（修复后） |
  | 在 `"wb"` 分支写到一半被取消 | `.tmp` 被删除，不存在 | 归零 | 已经在 `open()` 之后被覆盖成新值/`None`，不会再改回旧值 | 是——同上一行“失败”的理由：`.tmp` 不存在使 `offset` 为 0，这个校验子（无论新旧）同样是不可达的死值，且这个 `state` 因为不是“暂停”结局，`finished` 会被置为 `True`，不会被“继续”重新提交 |

  表格里没有一格允许“磁盘上是新正文，`validator` 却是旧值”或者反过来，只有两种被判定为“不可达但技术上不完全一致”的死值组合（失败/取消分支里保留的旧校验子），而这两种都被 `offset > 0` 这个前置门槛挡住，永远不会被读出来拼进下一次请求。
