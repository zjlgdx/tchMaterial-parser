...
- **穷举自查：按“谁碰过 `.tmp`” × “碰完之后 `finished` 是不是 `False`” 列出每一条会写入或删除 `.tmp` 的路径**，而不是按“下载循环的结局”分类——`add_bookmarks` 就是一条在下载循环*之外*单独碰 `.tmp` 的路径（是“正文与校验子不同源”这个物种的第四个变种藏身的地方），按循环结局分类会漏掉它；而且只有 `finished` 停留在 `False` 的路径才会被下一轮真的读回去拼请求，其余路径都会让任务终结（`finished = True`，不会被“继续”重新提交），所以行维度必须是“谁碰过 `.tmp`”，不是“循环怎么结束”。`failed_reason` 单独成列：这类“长度自洽、计数器自洽、内容却错误”的文件之所以危险，正是因为它们不会触发任何失败提示；同样是“取消/中止”，有的路径合法地不写 `failed_reason`（取消不算失败），有的路径必须写（下载不完整、响应不可信、收尾阶段出错），把两者压成一行会看不出这条界线本身有没有守住，所以单独成列逐行核对：

  | 写入/删除 `.tmp` 的路径 | 磁盘上的 `.tmp` | `validator` | `failed_reason` | `finished` | 下一轮会不会读到 |
  | --- | --- | --- | --- | --- | --- |
  | 排队中被取消 | 尽力删除（可能是上一轮暂停留下的；这一轮从未发起网络请求） | 不碰 | 不写（取消不算失败） | `True` | **不会** |
  | 排队中被暂停 | 不碰，这一轮从未发起网络请求 | 不碰 | 不写 | `False` | **会**——但没有任何新东西被写过，直接沿用进入这一轮之前本就自洽的状态 |
  | 响应到手后、真正打开文件前被取消 | 尽力删除（这次的判断结果从未被应用到磁盘） | 不碰 | 不写 | `True` | **不会** |
  | 响应到手后、真正打开文件前被暂停（坑 2） | 不碰，还是进入这一轮之前的旧内容 | 不碰，还是进入这一轮之前的旧值 | 不写 | `False` | **会**——但没有任何新东西被写过，这一行的自洽性单纯来自“沿用了上一轮本就自洽的状态” |
  | 响应不可信（`open_mode is None`） | 尽力删除 | 不碰 | 写（`download_failure_reason` 给出的原因） | `True` | **不会** |
  | `"ab"` 续传写入循环中途被暂停 | 旧前缀 + 本次已写入的追加字节，全部同属校验子标识的那个版本 | 不碰，保留旧值 | 不写 | `False` | **会**——校验子从未失效过，已写入的追加字节经服务端用 If-Range 确认过与它同源 |
  | `"wb"` 全新正文写入循环中途被暂停 | 本次响应体已写入的前缀（`wb` 已在 `open()` 时截断旧内容） | 已在 `open()` 那一刻覆盖成这次响应给出的新值（或 `None`） | 不写 | `False` | **会**——已写入的字节全部来自这次响应，与刚覆盖的新校验子同源 |
  | 写入循环中途被取消、且尚未下满全长（含分块循环里的清晰取消判定与断连导致的异常路径，两者效果相同；已知总长且已经下满的那一瞬间被取消，归下面“加书签 + 改名收尾阶段顺利完成”那一行） | 尽力删除（`os.remove` 包在 `except Exception: pass` 里，可能删不掉，例如 Windows 文件被占用） | 这一行的判定本身不改 `validator`：若这次走的是 `"ab"` 分支，退出时还是旧值；若走的是 `"wb"` 分支，早在更早的 `open()` 那一刻就已经被覆盖成新值/`None`，退出时是新值，不是旧值 | 不写（取消不算失败） | `True` | **不会** |
  | 下载不完整（EOF 但字节数不足，或真正的网络/写入异常） | 尽力删除 | 同上一行——`"ab"` 分支保留旧值，`"wb"` 分支已在 `open()` 时被覆盖成新值/`None` | 写 | `True` | **不会** |
  | 加书签 + 改名收尾阶段顺利完成（传输已经下满全长——包括停止恰好落在最后一块之后的那一瞬间；不论命中的是暂停、取消，还是什么都没命中，只要 `add_bookmarks`、`os.replace` 两步都没有抛异常） | `.tmp` 不再存在，已改名为 `save_path` | 已在 `open()` 时覆盖/保留成正确值 | 不写 | `True` | **不适用**——`.tmp` 本身已经不存在，`offset` 下一次会算成 0。走到这一刻传输已经确认完整、书签也已经写成功，这是一份完好的成果，一律按完成收尾：文件改名交付、`finished = True`、不写 `failed_reason`。取消只回收**尚未完整**的半成品，不回收已经完整的成果——因为用户在最后两秒点了取消就把它删掉，等于丢弃一份已经做完的工作。这一行覆盖了绝大多数情况：`finalizing` 只出现在 `except` 分支的判据里，没有异常就根本不会进 `except`，收尾阶段命中暂停或取消照样按完成收尾 |
  | 收尾阶段抛异常、且命中取消 | 尽力删除（`add_bookmarks` 可能已经把它整份重写过，也可能还没来得及重写，取决于异常抛出的具体时机） | 不碰 | 不写（取消不算失败） | `True` | **不会** |
  | 收尾阶段抛异常、且未命中取消（命中暂停，或真正的写入/系统异常） | 尽力删除（同上） | 不碰 | 写 | `True` | **不会**——`finalizing` 标记只在这一格发挥作用：堵住“暂停 + 异常”这一种组合的“暂停”出路，不会产生“`.tmp` 是书签版、但 `validator`/`finished=False` 仍描述服务端原始正文”这种活的、会被读回的不一致状态 |
...
- 在飞中被取消 → 中止写入，删除 `.tmp`，`downloaded_size`/`total_size` 清零，`finished = True`（取消不算失败，不写 `failed_reason`）。这一条说的是**传输尚未确认完整**的阶段——已写字节还没到 `Content-Length`（含服务端根本没给总长、无从判定完整的情况）；一旦已知总长且确实下满，取消就不再回收这份成果，而是和暂停一样按下面例外一走完成收尾，见上表“加书签 + 改名收尾阶段顺利完成”那一行。
- 排队中/在飞中被暂停 → 保留已写的 `.tmp`，不清零 `downloaded_size`，`finished` 保持 `False`——这是批次里唯一不终结的情况，也是让 `downloads_active()`（定义 `not all(finished)`，完全不用改）继续把“已暂停”识别为“批次仍然活跃”的关键。**例外一**：如果停止恰好落在最后一块写完之后（`total_size > 0` 且 `downloaded_size == total_size`），说明文件其实已经下完，只是还没来得及走到成功分支，这种情况按“完成”处理（改名、`finished = True`、不写 `failed_reason`）。这条例外对暂停与取消一视同仁，判据只有“已知总长且确实下满”这一条，与按下的是哪个按钮无关：既不留着一个内容已经齐全的 `.tmp` 装作还在暂停，也不把一份已经完整的成果当半成品删掉。反过来，`total_size` 为 0（服务端没给 `Content-Length`）时无从判定完整，不适用这条例外，暂停照常留下 `.tmp`、取消照常清理。**例外二**：一旦确认传输已经完整、进入“加书签（`add_bookmarks`）+ 改名（`os.replace`）”这个收尾阶段（用一个局部的 `finalizing` 标记表示，在调用 `add_bookmarks` **之前**置位，覆盖收尾阶段的全部两步），这个阶段的暂停只有在**紧接着又让这两步之一抛出异常**时才不落成“暂停”——单纯命中暂停、但 `add_bookmarks`/`os.replace` 都顺利跑完的情况（这是绝大多数情况）仍然按完成收尾：文件正常改名、`finished = True`、不写 `failed_reason`，`finalizing` 这个标记只在 `except` 分支的判据里出现，没有异常就根本不会进 `except`，暂停与否影响不到结果。只有当 `os.replace` 恰好失败（例如 Windows 上目标文件被阅读器/杀软占用）、且异常抛出时 `stop_reason()` 已经不是 `"cancelled"`（也就是命中了暂停，或是真正的系统异常）时，才会判定为失败并清理 `.tmp`——因为这时 `add_bookmarks` 很可能已经把 `.tmp` 整份重写过（字节内容、长度都变了，不再是服务端正文的前缀），如果仍按“暂停”处理，就会把这份已经不是服务端正文前缀的 `.tmp` 留在磁盘上、却让 `validator`/`downloaded_size`/`total_size` 继续描述服务端原始正文——下一轮“继续”会用书签版 `.tmp` 的实际长度当偏移、配上原始正文的校验子发起 Range 请求，服务端一旦认可就会把原始正文的尾巴接到书签版前缀后面。若异常抛出时 `stop_reason()` 是 `"cancelled"`，则走取消分支清理 `.tmp`，不写 `failed_reason`（取消不算失败），与是否 `finalizing` 无关。

这条“命中异常时按失败处理”的规则不区分本次下载有没有勾选书签——没有章节时 `add_bookmarks` 确实不会被调用、`.tmp` 也确实还是服务端正文的逐字节完整副本，但 `finalizing` 标记在进入收尾阶段时就已经统一置位，不为“有没有章节”单独分支：这种情况下即使按“暂停”处理，`.tmp` 大小已经等于完整长度，下一轮请求的 `Range` 起点会等于（或超过）资源总长，`Range` 本身已经不可满足，必然撞上 416——这与服务端内容有没有变化无关，纯粹是本地偏移已经越界；撞 416 之后照样会退化成一次全量重下，与直接判定失败清理 `.tmp` 的最终代价相当，但统一判定失败可以让“收尾阶段命中异常”这条规则只有一种表现，不需要在收尾阶段内部再区分“有没有重写过 `.tmp`”这层状态。

判定“为什么停下来”还有两条容易漏掉的时序规则，必须显式遵守：

- **先判定 `stop_reason()`，再判断响应可不可用**：请求返回之后，必须先检查 `stop_reason()`，命中暂停/取消就直接按暂停/取消收尾，根本不去看这次响应是不是可信——网络请求在飞的这段时间里随时可能被暂停/取消，此时响应内容是什么已经不重要，把它当失败处理（写 `failed_reason`）是错的，会把一次正常的暂停/取消误报成下载失败。
- **循环退出后必须重新读一次 `stop_reason()`，不能沿用循环内最后一次的值**：分块写入循环里每写完一块都会检查一次 `stop_reason()`，命中就 `break`；但循环退出后判定“这个文件最终算什么结局”时，必须**重新调用一次** `stop_reason()`，不能直接复用循环内触发 `break` 那次的返回值——两者之间可能存在流干净结束（EOF）与暂停/取消几乎同时发生的窗口，只有重新读一次才能配合上面“暂停恰好落在最后一块之后”的例外做出正确判断。
...
10. **下载阶段取消/暂停的生效延迟受限于当前这一条正在进行的下载请求（最坏情况到 `REQUEST_TIMEOUT` 的连接 10 秒 + 读 60 秒），是否需要强制中断这一条在飞的请求？** 默认：不做，接受“至多晚于一条在飞的 HTTP 请求”的延迟。理由：镜像轮换与 400 退避原本会把这个延迟叠乘成“镜像数 × 每个镜像的退避次数”条请求，这部分叠加已经由 (a) 里的停止检查点消掉，剩下的残余上界只剩当前这一条请求本身；响应头一旦到达，响应就登记进 `active_responses`，主线程的主动断连立刻生效，因此这个残余上界只存在于响应头到达之前的那一段。再往下压需要改动 `api.parse`/`network.session` 发起请求的方式（换成可以从外部中断的底层连接），与第 5 条同属与本需求无关的重构，收益（缩短一个罕见的尾延迟）远小于风险。
