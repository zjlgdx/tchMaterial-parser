...
- **穷举自查：按“谁碰过 `.tmp`” × “碰完之后 `finished` 是不是 `False`” 列出每一条会写入或删除 `.tmp` 的路径**，而不是按“下载循环的结局”分类——`add_bookmarks` 就是一条在下载循环*之外*单独碰 `.tmp` 的路径（坑 4 藏身的地方），按循环结局分类会漏掉它；而且只有 `finished` 停留在 `False` 的路径才会被下一轮真的读回去拼请求，其余路径都会让任务终结（`finished = True`，不会被“继续”重新提交），所以行维度必须是“谁碰过 `.tmp`”，不是“循环怎么结束”。`failed_reason` 单独成列：这类“长度自洽、计数器自洽、内容却错误”的文件之所以危险，正是因为它们不会触发任何失败提示；同样是“取消/中止”，有的路径合法地不写 `failed_reason`（取消不算失败），有的路径必须写（下载不完整、响应不可信、收尾阶段出错），把两者压成一行会看不出这条界线本身有没有守住，所以单独成列逐行核对：
  ...
  | 写入循环中途被取消（含分块循环里的清晰取消判定与断连导致的异常路径，两者效果相同） | 尽力删除（`os.remove` 包在 `except Exception: pass` 里，可能删不掉，例如 Windows 文件被占用） | 不碰 | 不写（取消不算失败） | `True` | **不会** |
  | 下载不完整（EOF 但字节数不足，或真正的网络/写入异常） | 尽力删除 | 不碰 | 写 | `True` | **不会** |
  | 加书签（`add_bookmarks`）+ 改名（`os.replace`）收尾阶段命中暂停或任何异常 | `add_bookmarks` 已把 `.tmp` 整份重写（字节内容、长度都变了，不再是服务端正文的前缀），随后尽力删除 | 不碰 | 写 | `True` | **不会**——`finalizing` 标记堵住了“暂停”这条出路，不会产生“`.tmp` 是书签版、但 `validator`/`finished=False` 仍描述服务端原始正文”这种活的、会被读回的不一致状态 |
  | `os.replace` 成功（收尾） | `.tmp` 不再存在，已改名为 `save_path` | 已在 `open()` 时覆盖/保留成正确值 | 不适用 | `True` | **不适用**——`.tmp` 本身已经不存在，`offset` 下一次会算成 0 |
