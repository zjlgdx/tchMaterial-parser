...
- **穷举自查：按“谁碰过 `.tmp`” × “碰完之后 `finished` 是不是 `False`” 列出每一条会写入或删除 `.tmp` 的路径**，而不是按“下载循环的结局”分类——`add_bookmarks` 就是一条在下载循环*之外*单独碰 `.tmp` 的路径（是“正文与校验子不同源”这个物种的第四个变种藏身的地方），按循环结局分类会漏掉它；而且只有 `finished` 停留在 `False` 的路径才会被下一轮真的读回去拼请求，其余路径都会让任务终结（`finished = True`，不会被“继续”重新提交），所以行维度必须是“谁碰过 `.tmp`”，不是“循环怎么结束”。`failed_reason` 单独成列：这类“长度自洽、计数器自洽、内容却错误”的文件之所以危险，正是因为它们不会触发任何失败提示；同样是“取消/中止”，有的路径合法地不写 `failed_reason`（取消不算失败），有的路径必须写（下载不完整、响应不可信、收尾阶段出错），把两者压成一行会看不出这条界线本身有没有守住，所以单独成列逐行核对：
  ...
  | 加书签 + 改名收尾阶段顺利完成（不论期间有没有命中暂停，只要 `add_bookmarks`、`os.replace` 两步都没有抛异常） | `.tmp` 不再存在，已改名为 `save_path` | 已在 `open()` 时覆盖/保留成正确值 | 不适用 | `True` | **不适用**——`.tmp` 本身已经不存在，`offset` 下一次会算成 0。这一行覆盖了绝大多数情况：`finalizing` 只出现在 `except` 分支的判据里，没有异常就根本不会进 `except`，暂停期间两步都成功照样按完成收尾 |
