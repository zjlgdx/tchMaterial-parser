...
**`core/cache.py`** — `load(cache_key) -> CatalogNode | None` / `load_any() -> tuple[str, CatalogNode] | None` / `store(cache_key, node)`。缓存文件是 `cache_dir()/catalog.json`，内容为 `{"version": <cache_key>, "tree": {...}}`，存的是裁剪后的树，约 2 MB。
...
- `reserve_unique_path(dir_path, base_name, ext) -> str` / `release_path(path)`：在一个模块级 `Lock` 保护的「已占用集合」里为并发任务分配互不冲突的路径，磁盘已存在的文件也算占用，冲突时依次尝试 ` (2)`、` (3)`……（实测同名教材多达 19 本，这是必须的）。
...
**(a) 并行拉取，串行解析。** 四个列表文件的**网络传输**并行（瓶颈在网络，`ThreadPoolExecutor(max_workers=4)` 把首次加载的墙钟时间压到四分之一），但「`json.loads` + 裁剪」必须串行化——用一把锁或一个单线程消费队列，保证同一时刻只有一份 10 MB 的中间对象存在。四路各自 `response.json()` 会让内存峰值达到约 **200 MB**；串行化解析后约 **80 MB**。解析只占 0.10 s，串行化几乎不损失时间，却省下 120 MB。
...
**(d) 缓存命中时完全不触碰那 40 MB。** 热启动只读约 2 MB 的裁剪后缓存文件。这正是把 `fetch_version()` 从 `fetch_tree()` 里拆出来的目的——版本探测必须廉价到可以无条件执行，缓存才有机会在付出 40 MB 之前拦下这次加载。
