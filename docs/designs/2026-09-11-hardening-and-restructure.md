...
10. 把版本探测拆成独立的廉价一步 `fetch_version()`，以它返回的版本标识为缓存键落盘，命中则秒开且完全不拉那四个列表文件；探测失败时回退到离线缓存。→ A3
...
- 模块持有自己的 `threading.Event` 取消标志，关窗时由 `ui/app.py` 置位；每拉完一个列表文件、每建完一批条目检查一次，避免关窗后目录线程还在闷头解析那约 32 MB（B7）。

**`core/cache.py`** — `load(cache_key) -> CatalogNode | None` / `load_any() -> tuple[str, CatalogNode] | None` / `store(cache_key, node)`。缓存文件是 `cache_dir()/catalog.json`，内容为 `{"version": <cache_key>, "tree": {...}}`，存的是裁剪后的树，实测约 0.98 MB。
...
- 后台资源树线程调用 `core.startup.load_catalog()`（A3、任务 9/10），该函数内部依次是：
  ...
  2. `cache.load(version)` 命中 → 直接用，**整个热启动路径不碰那四个列表文件**。
...
A3 的本质是数据量问题，所以约束要写死成数字而不是原则。

先分清两个数：**线路上的字节**与**解压后的字节**。上游对这几个列表文件开了 `br`，实测四个文件线路合计 **1.31 MB**，解压后 **31.86 MB**，压缩比 **24 倍**。下文所有 MB 除非特别说明，指的都是**解压后**的量——冷启动的代价压在解析、建树与内存上，不在流量上。按解压后的数字去估流量会高估二十倍。

实测**最大的那个**列表文件（四个文件解压后分别是 9.92 / 8.86 / 8.06 / 5.01 MB）：原始 **9.92 MB**；`json.loads` 本身只要 **0.10 s**（解析速度从来不是瓶颈），但解析后的对象常驻 **19.7 MB**、过程峰值 **39.2 MB**；裁剪到必要字段后只剩 **0.43 MB**，缩减 **23 倍**。拿它乘 4 会高估——四个文件合计是 31.86 MB，不是 39.7 MB。裁剪后整棵树的常驻量见下文基准：约 **1.4 MB**。下面四条是硬约束：

**(a) 并行传输，串行解析。** 四个列表文件的**网络传输**并行（瓶颈在网络），但「`json.loads` + 裁剪」在一把锁里串行，同一时刻只存在一份中间对象。

以下为实测：按上游规模合成 4 个各约 9.5 MB 的列表文件（合计 38 MB，比实测的 31.86 MB 大约 19%——基准偏大，下面这些数字因此是保守的上界），传输用一次真实的字节拷贝并叠加 0.15 s 传输耗时来模拟网络节奏，每个变体在独立子进程里跑 5 次，取中位数。内存以 `tracemalloc` 的 Python 分配峰值为主指标，`ru_maxrss` 增量为辅（后者含分配器高水位，抖动较大）。
...
冷启动的真实代价因此是**一次性约 67 MB 的峰值**（在上面那个偏大 19% 的基准上量的，实际规模下更低）；之后常驻的只有裁剪后的树，约 **1.4 MB**，落盘的缓存文件约 **0.98 MB**（4000 本课本）。全文提到「裁剪后的树」一律指这个 1.4 MB，提到「缓存文件」一律指 0.98 MB。
...
**(d) 缓存命中时完全不触碰那四个列表文件。** 热启动只读那个约 0.98 MB 的缓存文件。这正是把 `fetch_version()` 从 `fetch_tree()` 里拆出来的目的——版本探测必须廉价到可以无条件执行，缓存才有机会在付出那约 32 MB 的解析与建树之前拦下这次加载。
...
`tests/fixtures/` 下全部是**手工裁剪的 JSON**，每个文件控制在几 KB，来源是源码 `:41-62` 注释里记录的真实响应结构，以及现有代码对字段的实际使用（`ti_items` / `lc_ti_format` / `ti_storages` / `tag_paths` / `hierarchies` / `tag_id` / `tag_name` / `urls` / `title` / `name` / `id` / `resource_type_code`）。**不在测试期访问网络，也不把那几个真实列表文件入库。**
...
- 缓存要不要设过期时间？→ 默认**不设**。缓存键已经绑定了上游版本，加 TTL 只会在上游没变时白白重跑一次整树拉取——代价主要不在流量（线路约 1.3 MB），而在那约 32 MB 的解析、约 67 MB 的内存峰值和 4476 个节点的建树。另在「加载失败」提示里提供一个「强制刷新」入口，用户可手动废弃缓存。
