...
def sanitize_filename(title: str) -> str: # 把接口返回的标题变成安全的文件名
...
reserved_paths = set() # 已被本次批量下载占用的目标路径
reserved_paths_lock = threading.Lock()

def unique_path(dir_path: str, base_name: str, ext: str) -> str: # 为重名教材分配互不冲突的路径
...
def assert_within(dir_path: str, file_path: str) -> None: # 确认构造出的路径没有逃出所选文件夹
...
def download() -> None: # 下载资源文件
    ...
    for url in urls:
        ...
        if dir_path:
            save_path = unique_path(dir_path, default_filename, ".pdf") # 构造完整路径
            assert_within(dir_path, save_path)
        else:
